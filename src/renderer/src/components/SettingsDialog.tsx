...
function JiraSection(): React.JSX.Element {
  ...
  const authenticate = async (): Promise<void> => {
    ...
    try {
      await api.invoke('jira:authenticate')
  ...
  const disconnect = async (): Promise<void> => {
    try {
      await api.invoke('jira:disconnect')
      setTesting(null)
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    }
  ...
  return (
    <section className="mt-5">
      ...
      <p className="mb-3 text-[11px] text-muted">Lets "New workspace" start from a ticket and suggest a name.</p>
