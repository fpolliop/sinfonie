 ...
 */
import { paddle } from './_paddle.js'

export async function onRequestGet({ request, env }) {
  const txn = new URL(request.url).searchParams.get('_ptxn') || ''
  const ok = /^txn_[A-Za-z0-9]+$/.test(txn) && env.PADDLE_CLIENT_TOKEN
  // Paddle Retain wants the Paddle customer id when there is one; the transaction knows it.
  let customerId = null
  if (ok && env.PADDLE_API_KEY) customerId = (await paddle(env, 'GET', `/transactions/${txn}`).catch(() => null))?.customer_id || null
...
Paddle.Initialize({ token: ${JSON.stringify(env.PADDLE_CLIENT_TOKEN)}${customerId ? `, pwCustomer: { id: ${JSON.stringify(customerId)} }` : ''} });
