...
When you create a workspace, Orchestra creates a git worktree on the same branch in
...
```
~/orchestra/workspaces/checkout-redesign/
...
- Terminal tab: a shell per worktree with `CONDUCTOR_*` and `ORCHESTRA_*` env vars set
...
Orchestra reads the same file Conductor does, at the root of each repository:
...
Variables available to scripts and terminals, in both `CONDUCTOR_*` and `ORCHESTRA_*` spellings:
